...
void print_help()
{
    printf(
        ...
        "\t\tcase 10 to case 13 - performance test case, need other parameters:\n"
        "\t\t\t--case 10: hpl pattern test\n"
        "\t\t\t--case 11: single direction performance test\n"
        "\t\t\t--case 12: multi direction performance test\n"
        "\t\t\t--case 13: mixed scenario test (send/recv multi-thread)\n"
        "\t[--send]:\n"
        "\t\tcpu core id of send process (range: [0, 607])\n"
        "\t[--recv]:\n"
        "\t\tcpu core id of recv process (range: [0, 607])\n"
        "\t[--num-of-cpu]:\n"
        ...
        "\t[--src-stride]:\n"
        "\t\tsource stride length (range: [0, 4M], -S)\n"
        "\t[--dst-stride]:\n"
        "\t\tdestination stride length (range: [0, 4M], -D)\n"
        "\t[--stride-num]:\n"
        "\t\tstride number (range: [0, 4M], -N)\n"
        "\t[--chn-type]:\n"
...
static int sdma_test_getopt(int argc, char **argv, struct sdma_test_input *test_cmd)
{
    ...
    struct option long_opt[] = {
        ...
        {"src-stride", required_argument, NULL, 'S'},
        {"dst-stride", required_argument, NULL, 'D'},
    ...
    while ((c = getopt_long(argc, argv, "hac:s:r:n:d:l:p:m:i:S:D:N:C:T:R:", long_opt, NULL)) != -1) {
        switch (c) {
            ...
            case 's':
                ...
                if (test_cmd->send_cpu > 607) {
                    fprintf(stderr, "send_cpu out of range [0, 607]\n");
                    return SDMA_TEST_FAILED;
                }
                break;
            case 'r':
                test_cmd->recv_cpu = strtol(optarg, &endptr, DECIMAL);
                if (test_cmd->recv_cpu > 607) {
                    fprintf(stderr, "recv_cpu out of range [0, 607]\n");
                    return SDMA_TEST_FAILED;
                }
                break;
            case 'n':
            ...
            case 'i':
                ...
                break;
            case 'S':
                test_cmd->src_stride_len = strtol(optarg, &endptr, DECIMAL);
                if (test_cmd->src_stride_len > 4 * 1024 * 1024) {
                    fprintf(stderr, "src_stride_len out of range [0, 4M]\n");
                    return SDMA_TEST_FAILED;
                }
                break;
            case 'D':
                test_cmd->dst_stride_len = strtol(optarg, &endptr, DECIMAL);
                if (test_cmd->dst_stride_len > 4 * 1024 * 1024) {
                    fprintf(stderr, "dst_stride_len out of range [0, 4M]\n");
                    return SDMA_TEST_FAILED;
                }
                break;
            case 'N':
                test_cmd->stride_num = strtol(optarg, &endptr, DECIMAL);
                if (test_cmd->stride_num > 4 * 1024 * 1024) {
                    fprintf(stderr, "stride_num out of range [0, 4M]\n");
                    return SDMA_TEST_FAILED;
                }
                break;
            case 'C':
                test_cmd->chn_type = strtol(optarg, &endptr, DECIMAL);
                if (test_cmd->chn_type != 0 && test_cmd->chn_type != 1) {
                    fprintf(stderr, "chn_type must be 0 (shared) or 1 (exclusive)\n");
                    return SDMA_TEST_FAILED;
                }
                break;
            case 'T':
...
void fill_sdma_task(void *sqe, int task_num, int chn_num, int data_size, uint32_t process_id, char *src, char *dest)
{
    ...
    for (i = 0; i < task_num * chn_num; i++) {
        ...
        sqe_task[i].next_sqe = (i < task_num * chn_num - 1) ? &sqe_task[i + 1] : NULL;
    }
}
